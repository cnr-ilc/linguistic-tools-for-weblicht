...
public class TokenizerKafResource {
    ...
    private String CLASS_NAME = TokenizerKafResource.class.getName();
    private String context = "services/kaf";
    ...
    public StreamingOutput tokenizeTextFromPlainFromUrl(@QueryParam("lang") String lang, @QueryParam("url") String theUrl, final InputStream input) {
        ...
        Logger
                .getLogger(CLASS_NAME).log(Level.INFO, message);
        OutputStream tempOutputData = null;
        submessage = String.format("***** FOR LRS KAF -GET INVOCATION:\n\t"
                + "***** FOR LRS KAF -GET INVOCATION lang parameter -%s- ******\n\t"
                + "***** FOR LRS KAF -GET INVOCATION url parameter -%s- ******\n\t"
                + "***** FOR LRS KAF -GET INVOCATION END *****\n\n", lang, theUrl);
...
//        System.err.println("***** FOR LRS KAF -GET INVOCATION END *****\n\n");
    ...
    /**
     * This method processes the plain text and creates a KAF document from the
     * input provided. It calls the corresponding method from the tool.
     *
